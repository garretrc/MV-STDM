...
load("data/results/results_s1_rw_84_95_1500.RData")
load("data/results/results_s1_rw_84_95_1500_2.RData")
load("data/results/results_s1_uni_84_95_1500.RData")
load("data/results/results_s1_uni_84_95_1500_2.RData")
load("data/results/results_s1_multi_84_95_1500.RData")
load("data/results/results_s1_multi_84_95_1500_2.RData")
...
rmse(eps_rw[-t_s1,-s1_uni])
rmse(eps_uni[-t_s1,-s1_uni])
rmse(eps_multi[-t_s1,-s1_uni])
...
test_rmse = data.frame(
  RMSE = c(apply(eps_rw[t_s1,s1_uni],1,rmse),
           apply(eps_uni[t_s1,s1_uni],1,rmse),
           apply(eps_multi[t_s1,s1_uni],1,rmse)),
  fit = rep(c('Univariate-RW','Univariate','Multivariate'),each=length(t_s1)),
  time = rep(seq(as.Date("1991/8/1"), by = "month", length.out = length(t_s1)),3)
)
...
y_test_samps = y_samps[t_s1,s1_uni,]
s2_samps = results_rw$sigma[t_s1,]
me_samps = aperm(a = apply(s2_samps,1:2,function(s2){rnorm(length(s1_uni),0,sqrt(s2))}), perm = c(2,1,3))
ypred_samps = y_test_samps + me_samps
d = dim(ypred_samps)[1:2]
crps_rw = matrix(NA,d[1],d[2])
ypred_test = ypred_rw[t_s1,s1_uni]
for(t in 1:d[1]){
  for(i in 1:d[2]){
...
y_samps = abind::abind(y_samps,along=3)
y_test_samps = y_samps[t_s1,s1_uni,]
s2_samps = results_uni$sigma[t_s1,]
me_samps = aperm(a = apply(s2_samps,1:2,function(s2){rnorm(204,0,sqrt(s2))}), perm = c(2,1,3))
ypred_samps = y_test_samps + me_samps
d = dim(ypred_samps)[1:2]
crps_uni = matrix(NA,d[1],d[2])
ypred_test = ypred_uni[t_s1,s1_uni]
for(t in 1:d[1]){
  for(i in 1:d[2]){
...
y_samps = abind::abind(y_samps,along=3)
y_test_samps = y_samps[t_s1,s1_uni,]
s2_samps = results_multi$sigma[t_s1,3,]
me_samps = aperm(a = apply(s2_samps,1:2,function(s2){rnorm(204,0,sqrt(s2))}), perm = c(2,1,3))
ypred_samps = y_test_samps + me_samps
d = dim(ypred_samps)[1:2]
crps_multi = matrix(NA,d[1],d[2])
ypred_test = ypred_multi[t_s1,s1_uni]
for(t in 1:d[1]){
  for(i in 1:d[2]){
